...
def main():
	...
	for viaje_id, viaje_data in data["services"].items():
		...
		if nodo1_type == "D" and nodo2_type == "A":
			...
			G.add_edge(nodo1_time, nodo2_time, tipo="tren", demanda=viaje_data["demand"], capacidad= float("inf"))
		elif nodo1_type == "A" and nodo2_type == "D":
			# Crea la arista de tipo "tren" desde nodo2 a nodo1
			G.add_edge(nodo2_time, nodo1_time, tipo="tren", demanda=viaje_data["demand"], capacidad= float("inf"))
	...
	for estacion, nodos in estaciones_nodos.items():
		...
		for i in range(len(nodos_ordenados) - 1):
			G.add_edge(nodos_ordenados[i], nodos_ordenados[i + 1], tipo="traspaso")
		G.add_edge(nodos_ordenados[-1], nodos_ordenados[0], tipo="trasnoche")
